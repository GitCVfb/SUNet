...
parser.add_argument('--batch_sz', type=int, default=6, help='batch size used for training')#6
...
parser.add_argument('--lr_step', type=int, default=500, help='lr decay rate')
parser.add_argument('--lr_start_epoch_decay', type=int, default=100, help='epoch to start lr decay')
...
class Train(Generic_train_test):
    ...
    def validation(self, epoch):
        ...
        for i, data in enumerate(self.dataloader_val):
            ...
            # compute metrics 
            psnr_sum += PSNR(pred_im[-1], im_gs)
            ssim_sum += SSIM(pred_im[-1], im_gs)
