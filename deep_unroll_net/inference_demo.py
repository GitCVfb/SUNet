    ...
    def test(self):
        with torch.no_grad():
            ...
            for seq in seq_lists:
                ...
                pred_im, _, pred_flow=self.model.forward()
                ...
                io.imsave(os.path.join(self.opts.results_dir, seq+'_pred_1.png'), (pred_im[-1].clamp(0,1).cpu().numpy().transpose(0,2,3,1)[0]*255).astype(np.uint8))
